...
REM                                                                                                                    #
REM Args:     b creates backup(s) *.original in same directory (otherwise a backup is created in the TEMP folder)      #
REM           q suppresses progress report                                                                             #
REM           file is the file(s) to rotate, or directory containing photos                                            #
REM                                                                                                                    #
REM Notes:    Photos with Orientation 3/6/8 are rotated and their Orientation set to 1, others are skipped             #
REM           This script is for JPG images with EXIF data, whose width and height are divisible by 16                 #
REM           Older cameras record Orientation=1 for all photos, these must be rotated by hand (see jpgrot.bat)        #
REM           Wildcards must start with * (*.jpg, *2008.jpg), midstring wildcards are not supported (2008*.jpg)        #
REM                                                                                                                    #
...
rem Allow many file args and use recursion to handle dirs in a special way (can't use %* because includes shifted -b -q)
set backup=0
set quiet=0
set userargs=
...
rem %1 is a single valid filename ______________________________________________________________________________________
