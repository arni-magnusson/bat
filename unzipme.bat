...
REM                                                                            #
REM Args:     asis means unzip in current directory "as is", not creating a    #
REM             new directory for the output                                   #
REM           file is file to unzip                                            #
REM                                                                            #
REM Notes:    If filename includes ampersand &, a needless warning is shown    #
REM             and can be ignored                                             #
REM                                                                            #
REM Requires: 7za, bunzip2, compress, gawk, gunzip, lzma, tar, unrar, unzip,   #
REM             xz                                                             #
REM                                                                            #
REM Returns:  Creates unzipped files in current directory, or new directory if #
REM             archive contains many file/dirs                                #
REM                                                                            #
REM ############################################################################
...
7za l %1 | sed -n "/^[1-2]/p" | cut -c54- | gawk -F\ "{print $1}" |^
 uniq | wc -l > %lines%
...
for /F "usebackq tokens=*" %%L in ("%lines%") do set unique=%%L
if %unique%==1 (%tar% -xf  %1 & goto EOF)^
 else (md "%I:~0,-4%" & %tar% -C "%I:~0,-4%" -xf %1 & goto EOF)
...
for /F "usebackq tokens=*" %%L in ("%lines%") do set unique=%%L
if %unique%==1 (%tar% -xf %1 & goto EOF)^
 else (md "%I:~0,-8%" & %tar% -C "%I:~0,-8%" -xf %1   & goto EOF)
...
for /F "usebackq tokens=*" %%L in ("%lines%") do set unique=%%L
if %unique%==1 (%tar% -xf %1 & goto EOF)^
 else (md "%I:~0,-7%" & %tar% -C "%I:~0,-7%" -xf %1 & goto EOF)

:.TAR.7Z
rem The 'xz' program does not recognize (some?) 7z files
if %asis%==1 7za x -so %1 | %tar% -x & goto EOF
7za x -so %1 2> NUL | %tar% -t | sed "s/^\.\///" | gawk -F/ "{print $1}" |^
 uniq | wc -l > %lines%
for /F "usebackq tokens=*" %%L in ("%lines%") do set unique=%%L
if %unique%==1 (7za x -so %1 | %tar% -x & goto EOF)^
 else (md "%I:~0,-7%" & 7za x -so %1 | %tar% -C "%I:~0,-7%" -x & goto EOF)

:.TAR.XZ
rem XZ format not supported in Windows tar 1.21 (Rtools)
rem     if %asis%==1 %tar% -xf %1 & goto EOF
if %asis%==1 xz -dc %1 | %tar% -x & goto EOF
rem            %tar% -tf %1 | sed "s/^\.\///" | gawk -F/ "{print $1}" |^
rem uniq | wc -l > %lines%
xz -dc %1 | %tar% -t 2> NUL | sed "s/^\.\///" | gawk -F/ "{print $1}" |^
 uniq | wc -l > %lines%
for /F "usebackq tokens=*" %%L in ("%lines%") do set unique=%%L
rem if %unique%==1 (%tar% -xf %1 & goto EOF)^
rem else (md "%I:~0,-7%" & %tar% -C "%I:~0,-7%" -x %1 & goto EOF)
if %unique%==1 (xz -dc %1 | %tar% -x & goto EOF)^
 else (md "%I:~0,-7%" & xz -dc %1 | %tar% -C "%I:~0,-7%" -x & goto EOF)
...
for /F "usebackq tokens=*" %%L in ("%lines%") do set unique=%%L
if %unique%==1 (xz -dc %1 | %tar% -x & goto EOF)^
 else (md "%I:~0,-9%" & xz -dc %1 | %tar% -C "%I:~0,-9%" -x & goto EOF)
...
for /F "usebackq tokens=*" %%L in ("%lines%") do set unique=%%L
if %unique%==1 (%tar% -xf %1 & goto EOF)^
 else (md "%~dpn1" & %tar% -C "%~dpn1" -xf %1 & goto EOF)
